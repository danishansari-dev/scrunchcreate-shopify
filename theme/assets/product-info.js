  ...
  customElements.define(
    ...
    class ProductInfo extends HTMLElement {
      ...
      connectedCallback() {
        ...
        this.initActiveVariantMedia();
        this.dispatchEvent(new CustomEvent('product-info:loaded', { bubbles: true }));
      }

      initActiveVariantMedia() {
        const variant = this.getSelectedVariant(this);
        if (variant) {
          this.updateMedia(null, variant?.featured_media?.id, variant);
        }
      }
      ...
      updateMedia(html, variantFeaturedMediaId, variant) {
        const mediaGallery = this.querySelector('media-gallery');
        if (!mediaGallery) return;

        let targetMediaId = variantFeaturedMediaId ? `${this.dataset.section}-${variantFeaturedMediaId}` : null;

        // Resolve target media ID by matching variant option/color
        if (variant) {
          const color = (variant.option1 || variant.title || '').toLowerCase().trim();
          if (color) {
            const allItems = Array.from(this.querySelectorAll('media-gallery li[data-media-id]'));
            const matched = allItems.find((item) => {
              const src = (item.dataset.mediaSrc || item.querySelector('img')?.src || '').toLowerCase();
              return src.includes(`-${color}-`) || src.includes(`/${color}/`) || src.includes(`-${color}.`) || src.includes(color);
            }) || allItems.find((item) => {
              const alt = (item.dataset.mediaAlt || item.querySelector('img')?.alt || '').toLowerCase();
              return alt.includes(color);
            });
            if (matched) {
              targetMediaId = matched.dataset.mediaId;
            }
          }
        }
        ...
        if (targetMediaId) {
          mediaGallery.setActiveMedia?.(targetMediaId, true);
        }
